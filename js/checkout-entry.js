...
function CheckoutApp() {
    ...
    useEffect(() => {
        ...
        // Config de tienda (marca, zonas, datos de pago) — del cache o de Firestore
    ...
    const handleProceedToPayment = (e) => {
        ...
        setPendingOrder({
            ...
            discountAmount,
            shippingZone: activeZone ? activeZone.label : '',
    ...
    const handleFinalizeOrder = async () => {
        ...
        try {
            ...
            m += `*PRODUCTOS:*\n`;
            pendingOrder.items.forEach(i => { m += `▪️ ${i.title} (${i.selectedSize}) - ${fmtPrice(getPrice(i))}\n`; });
            m += `\n*Subtotal:* ${fmtPrice(pendingOrder.subtotal)}\n`;
            if (pendingOrder.discountAmount > 0) m += `*Descuento (${pendingOrder.discountCode}):* - ${fmtPrice(pendingOrder.discountAmount)}\n`;
            m += `*Envío:* ${fmtPrice(pendingOrder.shippingCost)}\n`;
            m += `*TOTAL PAGADO:* ${fmtPrice(pendingOrder.total)}\n\n`;
                    ...
                    <div className="p-6">
                        ...
                        <div className="text-xs font-mono text-zinc-500 mb-6 space-y-1 border-t border-zinc-900 pt-4">
                            ...
                            <div className="flex justify-between text-kuraRed font-bold text-sm mt-2 pt-2 border-t border-zinc-900">
                                <span>TOTAL:</span><span>{fmtPrice(confirmedOrder.total)}</span>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                    ...
                    <div className="space-y-4 md:sticky md:top-24">
                            ...
                            {cart.map(item => (
                                <div key={item.cartId} className="flex gap-3 border border-zinc-800 p-3 bg-zinc-950 rounded-xl">
                                    ...
                                    <div className="flex-1 overflow-hidden">
                                        ...
                                        <p className="text-kuraRed text-sm font-bold mt-1">{fmtPrice(getPrice(item))}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="bg-zinc-950 border border-zinc-800 p-4 font-mono text-sm rounded-xl">
                            ...
                            <div className="flex justify-between mb-2 text-zinc-400"><span>ENVÍO ({pendingOrder ? pendingOrder.shippingZone : (activeZone?.label || '')})</span><span>{fmtPrice(pendingOrder ? pendingOrder.shippingCost : currentShippingCost)}</span></div>
                            <div className="flex justify-between pt-3 border-t border-zinc-800 items-end mt-2">
                                ...
                                <span className="font-bebas text-3xl text-kuraRed leading-none">{fmtPrice(pendingOrder ? pendingOrder.total : cartTotal)}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        {!pendingOrder ? (
                            ...
                            <div className="space-y-4 animate-slideUp">
                                ...
                                <div className="bg-zinc-950 border border-zinc-800 p-5 rounded-2xl relative overflow-hidden">
                                    ...
                                    <div className="bg-black border border-zinc-800 p-4 text-xs text-zinc-300 font-mono rounded-xl">
                                        ...
                                        {storeConfig.paymentInfo ? (
                                            <p className="whitespace-pre-wrap leading-relaxed">{storeConfig.paymentInfo}</p>
                                        ) : (
                                ...
                                <label className="flex items-start gap-3 cursor-pointer text-xs text-zinc-400 p-3 border border-zinc-800 bg-black rounded-xl">
                                    ...
                                    <span className="leading-tight">Acepto la <a href="#" className="text-white underline">política de envíos</a> de {branding.brandName}.</span>
                                </label>
